...
-- migration meant. SQLite cannot add a CHECK constraint to an existing table
-- without rewriting it, so the allowed values are enforced by the store rather
-- than the schema; Postgres carries the constraint.

ALTER TABLE pending_objects ADD COLUMN role TEXT NOT NULL DEFAULT 'primary';
...
CREATE INDEX IF NOT EXISTS idx_pending_objects_key ON pending_objects (object_key);
